...
class nfa:
    ...
    def add_transition(self, from_state, to_state, input):
        if not((from_state in self.states) and (to_state in self.states)):
            raise Exception('Make sure all specified states exist in the NFA.')
        elif input not in alphabet:
            raise Exception('Invalid input parameter.')
        else:
            self.transitions[from_state][input] = to_state
    ...
    def to_numeric_states(self, starting_index):
        ...
        for state, index in new_indices.items():
            for input, to_state in self.transitions[state].items():
                new_nfa.add_transition(index, new_indices[to_state], input)
    ...
    def to_json(self):
        ...
        for state in nfa.states:
            ...
            for input, to_state in nfa.transitions[state].items():
                s[input] = 'S'+str(to_state)
